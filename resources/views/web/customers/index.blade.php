    ...
    <form method="POST" action="{{ route('customers.store') }}" class="mt-10 space-y-5">
        ...
        <div class="grid grid-cols-2 gap-5">
            ...
            <div>
                ...
                <div class="mt-1">
                  <input type="text" name="email" id="email" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-md" value="{{ old('email') }}">
                </div>
            </div>
            <div>
                <label for="sex" class="block text-sm font-medium text-gray-700">Sex</label>
                <div class="mt-1">
                  <input type="checkbox" name="sex" id="sex" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-md" {{ old('sex') ? 'checked' : '' }}>
                </div>
            </div>
            <div>
                <label for="is_send_notify" class="block text-sm font-medium text-gray-700">Send notifications</label>
                <div class="mt-1">
                  <input type="checkbox" name="is_send_notify" id="is_send_notify" class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-gray-300 rounded-md" {{ old('is_send_notify') ? 'checked' : '' }}>
                </div>
            </div>
        </div>
        <x-button>
